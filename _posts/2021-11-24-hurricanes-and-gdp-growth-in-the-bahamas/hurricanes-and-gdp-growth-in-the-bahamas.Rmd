...
summary(lm(gdp_growth ~ num_yr0 + num_yr1 + num_yr2 + num_yr5, data = hgdp))

gdp_num_strength <- lm(gdp_growth ~ num_yr0 + num_yr1 + num_yr2 + num_yr5, data = hgdp) %>%
  summary() %>% 
  xtable() %>% 
  kable(digits = 3, 
        caption = "Multiple Linear Regression of Number of Hurricanes and Lag Years on Annual GDP Growth") %>%
  kable_styling(latex_options = "HOLD_position")

gdp_num_strength

mod <- lm(gdp_growth ~ num_yr0 + num_yr2 + num_yr5, data = hgdp)
```
...
yr2 <- hgdp %>% 
  ...
  stat_smooth(method = "lm", se = FALSE) + 
  labs(x = "Average Number of Storms",
       y = "Annual % GDP Growth year 2") + 
  theme_minimal()

yr5 <- hgdp %>% 
  ggplot(aes(y = gdp_growth, x = num_yr5)) +
  geom_point() +
  stat_smooth(method = "lm", se = FALSE) + 
  labs(x = "Average Number of Storms",
       y = "Annual % GDP Growth year 5") + 
  theme_minimal()
